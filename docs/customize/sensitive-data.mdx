...
You should also configure [`BrowserSession(allowedDomains=...)`](https://docs.browsernode.com/customize/browser-settings#allowedDomains) to prevent the Agent from visiting URLs not needed for the task.
...
- Always restrict [`BrowserSession(allowedDomains=[...])`](https://docs.browsernode.com/customize/browser-settings#allowedDomains) to only the domains the agent needs to visit to accomplish its task. This helps guard against prompt injection attacks, jailbreaks, and LLM mistakes.
...
- Don't use `sensitiveData` for login credentials, it's better to use [`storageState`](docs.browsernode.com/customize/browser-settings#storageState) or a [`userDataDir`](/customize/browser-settings#user-data-dir) to log into the sites the agent needs in advance & reuse the cookies:
...
It's recommended to set `Agent(useVision=false)` when working with `sensitiveData`.
...
Domain patterns in `sensitiveData` follow the same format as [`allowedDomains`](https://docs.browsernode.com/customize/browser-settings#allowedDomains):
...
For convenience the system will validate that all domain patterns used in `Agent(sensitiveData)` are also included in `BrowserSession(allowedDomains)`.
...
const agent = new Agent(
    "Log into Google, then check my account information",
    llm,
    {
