...
const agent = new Agent("fill out the form on this page", browserSession);
```
...
- `BrowserSession(**params)` is browsernode's object that tracks a connection to a running browser. It sets up:
...
See here for [more detailed usage instructions](https://docs.browsernode.com/customize/real-browser#method-b%3A-connect-using-existing-playwright-objects).
...
Multiple running browsers **cannot share a single `userDataDir` at the same time**. You must set it to `None` or
...
This option is only applied when launching a new browser using the default builtin playwright chromium and `userDataDir=None` is set.
...
# then setup a BrowserSession with storageState='./storageState.json' and userDataDir=None to use it
```
